function FED = readExperimentalData_v6(TR,INDEXING)
% TR is instance of ScanProcessingClass
% Can also be used if TR is a Struct but may fail if fields are missing,
% but then use v6a

    FED.spotNumbers = INDEXING.roiNumbers;
    if ~isempty(TR.Intensities_P)
        FED.I_P = TR.Intensities_P(:,FED.spotNumbers);
    else
        FED.I_P = [];
    end
    if ~isempty(TR.Intensities_PP)
        FED.I_PP = TR.Intensities_PP(:,FED.spotNumbers);
    else
        FED.I_PP = [];
    end
    if ~isempty(TR.ratios)
