function refinedStruct = refineStructure_v10(FED, CRYSTAL, MODEL)
% Changes: 
% v3
...
for T = 1:nTimeDelays
    ...
    if ~MODEL.opt_fminunc
        options = optimset('Algorithm','levenberg-marquardt','Display','iter','MaxFunEvals',2e5,'TolX',1e-5,'MaxIter',1000);
        if MODEL.useBounds
            nPar = length(Pstart);
            LB = -ones(1,nPar);
    ...
    elseif MODEL.opt_fminunc
        options = optimset('Algorithm','quasi-newton','Display','iter','MaxFunEvals',2e5,'TolX',1e-5,'MaxIter',1000);
...
%     end
    ...
    function Fhkl = calcAllFhkl(uvw,U)
        ...
        DW_at = exp(-2*pi^2*Umesh.*modQ2rdmesh);
        P_at = cos(2*pi*uvw*HKLt);
        F_uvw_hkl = Fatomic_all.*DW_at.*P_at;
        Fhkl = sum(F_uvw_hkl);
    end
...
%         dXdotK = k0_unit*dXYZ'; Use to restrain in beam direction
        XYZ = dXYZ + XYZgs_free;
        XYZtemp(freeAtoms,:) = XYZ;
        dR = (atmPairs2dists(MODEL.restraints.atomPairs,XYZtemp) - DistListGS)';
