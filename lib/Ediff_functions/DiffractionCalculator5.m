classdef DiffractionCalculator5
...
% is in the z-direction: k0 = (0,0,1) 
    ...
    methods
        % Constructor
        function obj = DiffractionCalculator5(CRYSTAL,EXPERIMENT,MODEL)
            obj.crystal = CRYSTAL;
...
%             obj.modqIMG_list = modqIMG_L;
        ...
        function obj = calculateWTS_polar(obj,use000)
            ...
            for indx = 1:obj.nHKL
                ...
                sig_qRad_2rd = (obj.model.thetaRadial*modq)^2 + 0.5*dqPix2rd;
                sig_qRot_2rd = (obj.model.thetaRotation*modq)^2 + 0.5*dqPix2rd;
...
%                     %Correct az values for errors of 2*pi
                ...
                if ~(indx == obj.indxHKL000)
                    ...
                    Normalization = dqPix2rd/sqrt(sig_qRad_2rd*sig_qRot_2rd);
                elseif use000
                    argExp = dq_rad.^2*0.5/sig_qRad_2rd;
                    Normalization = dqPix2rd/sig_qRad_2rd;
                end
        ...
        function obj = calcIhkl_GS(obj)
            obj.Fhkl_GS = obj.calcFhkls(obj.UVW_GS, obj.Uiso_GS);
        ...
        function obj = runSim_GS(obj,options)
            ...
            if isempty(obj.UVW_GS)
                obj.UVW_GS = obj.model.UVW_GS;
            end
            
            if isempty(obj.Uiso_GS) && isfield(obj.model,'Uiso_GS')
                obj.Uiso_GS = obj.model.Uiso_GS;
            end
        ...
        function displaySqrtIMG(obj)
            ...
            imagesc(sqrtIMG,[0,max(max(sqrtIMG))]); 
            pbaspect([obj.experiment.imgsize.x,obj.experiment.imgsize.y,1]); 
            colormap(flipud(gray));
        end
