function IMG = projectCoordsGeneral_withOccup(XYZ,atoms,occupancies,ViewVector,VerticalDirection,X,Y,atomSize)
...
for a = 1:nY
    for b = 1:nX
        for atm = 1:nAt
            ...
            IMG(a,b) = IMG(a,b)+occupancies(atm)*atoms(atm)*exp(-(dX^2 + dY^2)/(atomSize^2));
        end
    end
end
end
