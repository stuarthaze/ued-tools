function refinedStruct = refineStructure_v8(FED, CRYSTAL, MODEL)
% Changes: 
% v3
...
% % parList2restraints = @(X) MODEL.restraints.lambdaPos*fPositionPenaltyLinear(calcR(fPositionChanges(parList2positions(X))));
% if MODEL.restraints.lambdaPos && ~(MODEL.restraints.lambdaBeamDirection)
%     parList2restraints = @(PX) MODEL.restraints.lambdaPos*PX(parIndicesFreeX)*T_par2dx;
% elseif ~MODEL.restraints.lambdaPos && MODEL.restraints.lambdaBeamDirection
%     parList2restraints = @(PX) MODEL.restraints.lambdaBeamDirection*dxInBeamDirection(PX);
% elseif MODEL.restraints.lambdaPos && MODEL.restraints.lambdaBeamDirection
%     parList2restraints = @(PX) [MODEL.restraints.lambdaPos*PX(parIndicesFreeX)*T_par2dx, MODEL.restraints.lambdaBeamDirection*dxInBeamDirection(PX)];
% else
%     parList2restraints = @(PX) [];
% end
...
%     end
    ...
    function Restraints = parList2restraints(PAR)
        ...
        Restraints = [MODEL.restraints.lambdaPos*PAR(parIndicesFreeX)*T_par2dx,...
