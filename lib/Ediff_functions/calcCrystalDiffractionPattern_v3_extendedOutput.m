function [IMG, Wts_ijh, HKL_list, Fhkls] = calcCrystalDiffractionPattern_v3_extendedOutput(EXPERIMENT,CRYSTAL,MODEL)
...
%
% NOTE: Coodinates are row vectors, so to convert bases use V2 = V1*T12
...
nSigmaCutoff = 4;
qCutoff = nSigmaCutoff*calcMaxSigmaParallel()
...
sigmaRad2rd = (MODEL.thetaRadial*modqIMG_L).^2 + dqPix^2;
sigmaPar2rd = (MODEL.thetaParallel*modqIMG_L).^2 + (beamDivSigma*modqIMG_L).^2 + MODEL.sigmaEwald^2;
sigmaRot2rd = (MODEL.thetaRotation*modqIMG_L).^2  + dqPix^2;
...
tic
disp('calculating HKL contributions')
Wts_hkl_pix = generateWeightMatrix();
toc



IMG_1d = (Ihkls')*Wts_hkl_pix;
%-----------------------------------------------
% Output  
IMG = imgaussfilt(reshape(IMG_1d,[imgsize.y,imgsize.x]),spotSizePixSigma);
Wts_ijh = reshape(Wts_hkl_pix,[imgsize.y,imgsize.x,nHKL]);

%--------------------------------------------------------
%            Nested Functions
%--------------------------------------------------------
%     function IMG_2d = calcIMGfromWeights(I_hkl,Wt_h_px)
%         IM1 = zeros([1,nPix]);
%         for X = 1:nPix
%             IM1(X) = I_hkl*

    function Wts_2d = generateWeightMatrix()
        Wts_2d = zeros(nHKL,nPix);
        for indx = 1:nHKL
            ...
            Wts_2d(indx,:) = Normalization.*exp(-argExp);
        end
    end
...
%         dQ_all = zeros(nhkl,npix,3);
    ...
    function maxSigmaParallel = calcMaxSigmaParallel()
        max_qIMG = max(modqIMG_L);
        maxSigmaParallel = sqrt((MODEL.thetaParallel*max_qIMG)^2 + (beamDivSigma*max_qIMG)^2 + MODEL.sigmaEwald^2);
    end
end