...
function [mod_f, phase_f] = interp_f_from_tables_EDICO (Z, s, E, pathname)
    if nargin == 3
        pathname = '\\win.desy.de\home\hayess\My Documents\MATLAB\lib\Ediff_functions\EDICO_scattering_factors_RelativisticSchrodinger';
    end
    ...
    for ii = 1: size(files,1)
        energy = regexp(files(ii).name, searchPattern, 'match');
        Energy_list(ii) = str2double(energy{1});
    end
    ...
    for ii = 1: length(Energy_list)-1
        % selecting the two energies it lies between
        if ( (E > Energy_list(ii)) && (E < Energy_list(ii+1)) )
            two_energies = [Energy_list(ii) Energy_list(ii+1)];
            fname1= [pathname, '\SCATFAC_', num2str(Energy_list(ii)), 'keV.txt'];
            fname2= [pathname, '\SCATFAC_', num2str(Energy_list(ii+1)), 'keV.txt'];
