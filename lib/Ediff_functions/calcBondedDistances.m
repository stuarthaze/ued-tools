function [bondedAtomPairs, bondedDistances] = calcBondedDistances(CRYSTAL,MODEL,FitResults,cutoff)
...
for T = 1:nT
    XYZ_cart = fractionals2cartesians(XYZ_T(:,:,T),CRYSTAL.axes);
    L = 1;
    for indxA = 1:(nAtFree-1)
        for indxB = (indxA+1):nAtFree
            ...
            atomPairs(L,:) = [A, B];
            dists(L,T) = fCalc_dR(XYZ_cart(A,:)-XYZ_cart(B,:));
            L = L + 1;
        end
    end
end
