...
%         useOutlierFilter
%         outlierThreshold
%         useNearestNeighbourFilter
%         thresholdNearestNeighbour
...
%--------------------------------------------------------------   
    methods
        %Constructor
        function obj = ScanProcessingClass(dataDir,ImageClass)
            obj.dataDir = dataDir;
...
%--------------------------------------------------------------------------
        function obj = readScanAveragedImages(obj,options)
            ...
            if options.readPump
                ...
                [IMGs_Pump, ~] = obj.readAveragedImagesSorted('*Pump *');
            end
            ...
            if options.subtractPumpBG
                if options.readPump
                    obj.IMGs_PP = obj.IMGs_PP - IMGs_Pump;
                else
                    for T = 1:ntPP
...
%                 end
                ...
                errorsROIsP = abs(ratioP-1) >= errorThreshold;
                errorsROIsPP = abs(ratioPP-1) >= errorThreshold;
                errorsROIs = errorsROIsP | errorsROIsPP;
                goodframesROIs = ~logical(sum(errorsROIs,1));

                % Check for errors based on sum(img) 
                imgSums_P = sum(sum(imgsProbe,2),1);
                imgSums_PP = sum(sum(imgsPP,2),1);
                imgSums_dI = sum(sum(imgsDiffs,2),1);

                med_P = median(imgSums_P);
                med_PP = median(imgSums_PP);
                med_diff = median(imgSums_dI);
        ...
        % Statistics
        %------------------------------------------------------------------
        function obj = calcRelativeChanges(obj)
            obj.RelativeChanges = obj.Intensities_dI./obj.Intensities_P;
            if ~isempty(obj.Uncertainties_dI)
                obj.Uncertainties_rel = obj.Uncertainties_dI./obj.Intensities_P;
            end
        end
...
%         end
        ...
        function showAverageDifferenceImage(obj,imgIndices,SCALE)
