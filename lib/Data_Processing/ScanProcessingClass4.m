classdef ScanProcessingClass4
% v3 - Modified outlier algorithm for finding single frame errors
% v4 - Included alternative file reading for macos
...
%--------------------------------------------------------------   
    methods
        %Constructor
        function obj = ScanProcessingClass4(dataDir,ImageClass)
            obj.dataDir = dataDir;
...
%                         end
            ...
            obj.sumImgErrors_P = zeros(NT,obj.nframes);
            obj.sumImgErrors_PP = zeros(NT,obj.nframes);
            obj.sumImgErrors_dI = zeros(NT,obj.nframes);
            obj.timeDelays = zeros(1,NT);
...
%                 % Check for errors based on roi values
%                 errorThreshold = 3;
...
% Outlier detection on images
% New v3
% 1. Calculate median images
% 2. Take sum of absolute differences from the median
% 3. Use median value as an estimate of expected error
% 4. Find outliers by taking a threshold based on #3.
                imgmed_P = median(imgsProbe,3);
                imgmed_PP = median(imgsPP,3);
                imgmed_dI = median(imgsDiffs,3);
                frameerrors_P = zeros(size(imgsProbe));
                frameerrors_PP = zeros(size(imgsProbe));
                frameerrors_dI = zeros(size(imgsProbe));
                for frame = 1:obj.nframes
                    frameerrors_P(:,:,frame)  = imgsProbe(:,:,frame) - imgmed_P;
                    frameerrors_PP(:,:,frame) = imgsPP(:,:,frame) - imgmed_PP;
                    frameerrors_dI(:,:,frame) = imgsDiffs(:,:,frame) - imgmed_dI;
                end
                
                sumFrameErr_P  = reshape(sum(abs(frameerrors_P),[1,2]),[1,obj.nframes]);
                sumFrameErr_PP = reshape(sum(abs(frameerrors_PP),[1,2]),[1,obj.nframes]);
                sumFrameErr_dI = reshape(sum(abs(frameerrors_dI),[1,2]),[1,obj.nframes]);
                
                errorThreshold = 2;
                goodFrames_P  = sumFrameErr_P  <= median(sumFrameErr_P)  + errorThreshold*(median(sumFrameErr_P) -min(sumFrameErr_P));
                goodFrames_PP = sumFrameErr_PP <= median(sumFrameErr_PP) + errorThreshold*(median(sumFrameErr_PP)-min(sumFrameErr_PP));
                goodFrames_dI = sumFrameErr_dI <= median(sumFrameErr_dI) + errorThreshold*(median(sumFrameErr_dI)-min(sumFrameErr_dI));
                
                goodframes = goodFrames_P & goodFrames_PP & goodFrames_dI;
                
                obj.sumImgErrors_P(tIndex,:) = sumFrameErr_P;
                obj.sumImgErrors_PP(tIndex,:) = sumFrameErr_PP;
                obj.sumImgErrors_dI(tIndex,:) = sumFrameErr_dI;
...
%                 goodframes = goodframes & goodframesROIs;
                ...
                if numGoodFrames == 0
                    disp(['Error: No good frames detected at t = ,',num2str(obj.timeDelays(tIndex))]);
                    goodFrames_P
                    goodFrames_PP
                    goodFrames_dI
%                     goodframesROIs
                ...
                % Use outlier filter on good frames
                if obj.filterBackupProcessed
                    for iter = 1:obj.filterBackupProcessed
                        imgsProbe(:,:,goodframes) = outlierFilterGeneral(imgsProbe(:,:,goodframes),obj.filterOptionsBackupProcessed);
                        imgsPP(:,:,goodframes) = outlierFilterGeneral(imgsPP(:,:,goodframes),obj.filterOptionsBackupProcessed);
                        imgsDiffs(:,:,goodframes) = outlierFilterGeneral(imgsDiffs(:,:,goodframes),obj.filterOptionsBackupProcessed);
                    end
                end
...
%             obj.IQR_T = obj.IQR_T(sortingIndices);
            ...
            obj.sumImgErrors_P = obj.sumImgErrors_P(sortingIndices,:);
            obj.sumImgErrors_PP = obj.sumImgErrors_PP(sortingIndices,:);
            obj.sumImgErrors_dI = obj.sumImgErrors_dI(sortingIndices,:);
            if obj.useDynamicPumpBG
...
%         end
        ...
        function IMGs = readBackupFramesSorted(obj,DirectoryPath,string2match)
            ...
            for indx = 1:numFiles
                ...
                if ismac
                    fullframe = readSPEnew(DirectoryPath,files(indx).name);
                else
        ...
        function [IMGs, timeDelaysSorted] = readAveragedImagesSorted(obj,string2match)
            ...
            for indx = 1:numFiles
                ...
                if ismac
                    fullframe = readSPEnew(obj.dataDir,files(indx).name);
                else
