function ScanData = processScan_v5_pumpBG_frameDiffs_detCalib(dataDir,ROIs3d,IMG_BG_det,IMG_BG_pump,IMG_Calibration,imgCrop,outlierThreshold,useDifferences)

imgSize = size(IMG_BG_det);
[NY_full,NX_full] = size(IMG_BG_det);
[NY_crop,NX_crop,Nspots] = size(ROIs3d);
backupDirs = dir(fullfile(dataDir,'Backup*'));
% Read first directory to find number of frames for pre-allocation of
% memory
directoryName_1 = [dataDir, '\', backupDirs(1).name];
filesPP_1 = dir(fullfile(directoryName_1, '*Pump+Probe*'));
numFrames = length(filesPP_1);
NT = length(backupDirs);
...
IMG_Calib_3d = zeros(NY_full,NX_full,numFrames);

for iFrame = 1:numFrames
    BG_3d_det(:,:,iFrame) = IMG_BG_det;
    BG_3d_pump(:,:,iFrame) = IMG_BG_pump;
    IMG_Calib_3d(:,:,iFrame) = IMG_Calibration;
end
...
for tIndex = 1:NT
    ...
    directoryName = [dataDir, '\', backupDirs(tIndex).name];
    imgs_t_P = (readImagesInFolderSorted(directoryName,'* Probe*',imgSize)-BG_3d_det).*IMG_Calib_3d;
    imgs_t_PP = (readImagesInFolderSorted(directoryName,'*Pump+Probe*',imgSize)-BG_3d_pump).*IMG_Calib_3d;
    ...
    goodframes_diff = (imgSums_diff <= (med_diff + outlierThreshold*IQR_diff)) & ...
        (imgSums_diff >= (med_diff - outlierThreshold*IQR_diff));

    numGoodFrames_P = sum(goodframes_P);
    numGoodFrames_PP = sum(goodframes_PP);
    numGoodFrames_diff = sum(goodframes_diff);
    

    if useDifferences
        IMGs_P(:,:,tIndex) = mean(imgs_t_P(:,:,goodframes_diff),3);
        IMGs_PP(:,:,tIndex) = mean(imgs_t_PP(:,:,goodframes_diff),3);
        Intensities_P(tIndex,:) = mean(spotIntensities_P(tIndex,:,goodframes_diff),3);
        Intensities_PP(tIndex,:) = mean(spotIntensities_PP(tIndex,:,goodframes_diff),3);
        Uncertainties_P(tIndex,:) = std(spotIntensities_P(tIndex,:,goodframes_diff),0,3)/sqrt(numGoodFrames_diff);
        Uncertainties_PP(tIndex,:) = std(spotIntensities_PP(tIndex,:,goodframes_diff),0,3)/sqrt(numGoodFrames_diff);
        numRejected_P(tIndex) = numFrames - numGoodFrames_diff;
        numRejected_PP(tIndex) = numFrames - numGoodFrames_diff;
        IQR_T(tIndex) = IQR_diff;
        usedFrames_P(tIndex,:) = goodframes_diff;
        usedFrames_PP(tIndex,:) = goodframes_diff;
    else
...
for tIndex = problemTimePoints
    ...
    directoryName = [dataDir, '\', backupDirs(tIndex).name];
    imgs_t_P = (readImagesInFolderSorted(directoryName,'* Probe*',imgSize)-BG_3d_det).*IMG_Calib_3d;
    imgs_t_PP = (readImagesInFolderSorted(directoryName,'*Pump+Probe*',imgSize)-BG_3d_det).*IMG_Calib_3d;
...
%     files_P = dir(fullfile(directoryName, '* Probe*'));
    ...
    med_diff = median(reshape(imageSumsAll_diff(nearestTimePoints,:),[numFrames*nAveraged,1]));
    ...
    goodframes_diff = (imgSums_diff <= (med_diff + IQRerrorThreshold)) & ...
        (imgSums_diff >= (med_diff - outlierThreshold*IQR_diff));
...
function IMGs = readImagesInFolderSorted(DirectoryPath,string2match,img_size)
    files = dir(fullfile(DirectoryPath,string2match));
    numFiles = length(files);
    IMGs = zeros([img_size, numFiles]);
    for indx = 1:numFiles
        frameNumber = str2double(regexp(files(indx).name,'(?<=frame )\d*','match'));
        IMGs(:,:,frameNumber) = double(DataIOLibrary.DataIO.ReadSpe(fullfile(DirectoryPath,files(indx).name)));
    end
end
