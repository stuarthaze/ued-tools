function viewImageWithCircles(IMG, intensityScaling, invertGrayscale, xCen, yCen, radii, textSize, textCellArr)
% Plots image with circles
% Set textSize = 0 to view without labels
% intensityScaling is a vector [min, max]
% invertGrayscale is a logical: 1 -> white is 0, black is maximum
...
for ii = 1:nCircles
    ...
    plot(x,y,'Color','g')
    if textSize
        text(xCenVec(ii)+rVec(ii),yCenVec(ii),textCellArr{ii},'FontSize',textSize,'Color',textColor);
    end
end
hold off;
end