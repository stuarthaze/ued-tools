...
% Distorts IMG_A
    ...
    methods
        function obj = ImageDistortionCalculator(imgA)
            ...
            %Initialize distortion Parameters for no distortion
            obj.distortionParams.intensityScale = 1;
            obj.distortionParams.baseline = 0;
            cen.y = size(imgA,1)/2;
            cen.x = size(imgA,2)/2;
            obj.distortionParams.cen = cen;
            obj.distortionParams.xshift = 0;
            obj.distortionParams.yshift = 0;
            obj.distortionParams.aspectRatio = 1;
            obj.distortionParams.aspectTheta = 0;
            obj.distortionParams.rotation = 0;
            obj.distortionParams.magnification = 1;
            obj.distortionParams.useRadialDistortion = 0;
            obj.distortionParams.radialQuad = 0;
            obj.distortionParams.radialCubic = 0;
            
        end
        function obj = resetDistortion(obj)
            obj.distortionParams.intensityScale = 1;
            obj.distortionParams.baseline = 0;
            obj.distortionParams.xshift = 0;
            obj.distortionParams.yshift = 0;
        ...
        function obj = loadNewIMG_A(obj,imgA)
            obj.IMG_A = imgA;
            obj.IMG_Aorig = imgA;
            if (size(imgA,1)~=obj.imgsizeA(1)) || (size(imgA,2)~=obj.imgsizeA(2))
                obj.imgsizeA = size(imgA);
                [obj.XX, obj.YY] = meshgrid(1:size(imgA,2),1:size(imgA,1));
                obj = obj.interpolateSizeAtoB();
            end
        end
        ...
        function obj = fitDistortionParams(obj)
            ...
            if obj.suppressOutput
                options = optimset('Display','final','MaxFunEvals',1e3);
            else
            ...
            if ~isempty(obj.useLsqnonlin) && obj.useLsqnonlin
                fun2minimize = @(X) (obj.MASK.*(obj.IMG_B - obj.refiningPars2IMG(X))).^2;
                LB = [];
                UB = [];
                pFit = lsqnonlin(@(pFit)fun2minimize(pFit),p0,LB,UB,options);
            else
