...
%             obj.imgsize = size(obj.IMG);
            ...
            %-- Radial BG
            obj = obj.calcRmatrix();
            if obj.subtractRadialBG
                obj.mask_bgSubtraction = zeros(size(obj.IMG)) > 1;
...
%             obj.peaks.vals = obj.peaks.vals(peaks2keep);
        ...
        %------------------------------------
        % Cropping
        function obj = setCroppingDetails(obj)
            obj.imgsize.x = obj.cropsize(2);
            obj.imgsize.y = obj.cropsize(1);
            if isempty(obj.cropping)
        ...
        function obj = filterIMG(obj,filterOpt)
            ...
            if filterOpt.medianOutlier
                imgfilt = medfilt2(obj.IMG);
                imgdiff = obj.IMG - imgfilt;
                noise = std(reshape(imgdiff,[1,size(obj.IMG,1)*size(obj.IMG,2)]));
                errors = abs(imgdiff) > filterOpt.medianOutlier*noise;
                obj.IMG(errors) = imgfilt(errors);
            end
...
%             figure();
            ...
            if isfield(displayOptions,'viewPeaks') && displayOptions.viewPeaks
