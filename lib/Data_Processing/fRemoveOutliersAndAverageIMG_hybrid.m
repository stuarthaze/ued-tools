function [IMG_2, num_good, IMG_stErrors] = fRemoveOutliersAndAverageIMG_hybrid(IMGs,threshold)
% Takes a 3d array, or stack of images, IMGs(Y,X,img#)
% Removes pixels that differ from (median+mean)/2 by more than
% threshold*(IQR+st.dev)
...
%         IMG_2(Y,X) = sum(good_pixels.*img_xy)/num_good(Y,X);        
