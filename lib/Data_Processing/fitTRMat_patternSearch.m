function Fit = fitTRMat_patternSearch(dataMat,timeDelays,numExp,numSin,freqStart)
...
if numSin
    ...
    Val0(ParIndx_SinDecays) = (timeDelays(end)-timeDelays(1));
end    
Val0(ParIndx_T0) = timeDelays(round(ny/3));             
...
%-------------------------------------------------
% Fitting
fitFun = @(Pars) sum(rms(dataMat - params2matrix_gen(Pars),2));

options = optimoptions('patternsearch','Display','iter','MaxFunEvals',5e4,'FunctionTolerance',1e-7);
% First fit only step, t0 and IRF
numExp_in = numExp;
numSin_in = numSin;
numExp = 0;
numSin = 1;
fittedPars1 = patternsearch(fitFun,Val0,[],[],[],[],[],[],options);
% Display results of initial fit
Fit1 = params2struct(fittedPars1)
% Add sin and exp decay components
numExp = numExp_in;
numSin = numSin_in;
fittedPars2 = patternsearch(fitFun,fittedPars1,[],[],[],[],[],[],options);
